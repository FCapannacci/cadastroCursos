...
export class ModulesController {
  ...
  @ApiResponse({ status: 409, description: 'Usuário já cadastrado', type: ConflictException })
  ...
  @ApiResponse({ status: 409, description: 'Nome de usuário já existente', type: ConflictException })
  ...
  @ApiResponse({ status: 201, description: 'Curso criado com sucesso', type: CursoSwagger })
  @ApiResponse({ status: 404, description: 'Professor não encontrado', type: NotFoundException })
  ...
  @ApiBody({ type: CursoSwagger })
  @ApiResponse({ status: 200, description: 'Curso editado com sucesso', type: CursoSwagger })
  @ApiResponse({ status: 404, description: 'Curso não encontrado', type: NotFoundException })
  async updateCurso(
  ...
  @ApiResponse({ status: 404, description: 'Curso não encontrado', type: NotFoundException })
  @ApiResponse({ status: 401, description: 'Apenas professores podem excluir cursos', type: UnauthorizedException })
  async deleteCurso(
    @Param('professorId') professorId: number,
    @Param('cursoId') cursoId: number,
  ) {
  ...
  @ApiBody({ type: [Number], description: 'Array de IDs de alunos, example: [1, 2, 3] ' })
  @ApiResponse({ status: 200, description: 'Acesso concedido com sucesso' })
  @ApiResponse({ status: 400, description: 'Digite um ID válido no corpo da requisição.', type: BadRequestException })
  @ApiResponse({ status: 404, description: 'Um ou mais alunos não foram encontrados.', type: NotFoundException })
  @ApiResponse({ status: 404, description: 'Curso não encontrado', type: NotFoundException })
  ...
  @ApiParam({ name: 'professorId', type: Number, description: 'ID do Professor' })
  @ApiParam({ name: 'cursoId', type: Number, description: 'ID do Curso' })
  @ApiResponse({ status: 200, description: 'Lista de alunos com acesso ao curso' })
  @ApiResponse({ status: 401, description: 'Apenas professores podem acessar esta informação.', type: UnauthorizedException })
  ...
  @ApiResponse({ status: 401, description: 'Aluno não atende aos critérios de aprovação', type: UnauthorizedException })
  async createAprovacao(@Body() aprovacaoDTO: AprovacaoDTO) {
    try {
      ...
      if (error instanceof UnauthorizedException) {
        throw new HttpException({ message: 'Aluno não atende aos critérios de aprovação', error: 'Unauthorized', statusCode: HttpStatus.UNAUTHORIZED }, HttpStatus.UNAUTHORIZED);
      } else if (error instanceof HttpException) {
        throw error;
      } else {
        throw new HttpException({ message: 'Aluno não atende aos critérios de aprovação' }, HttpStatus.INTERNAL_SERVER_ERROR);
      }
    }
  }
